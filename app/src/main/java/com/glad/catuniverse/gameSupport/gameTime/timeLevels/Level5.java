...
import static com.glad.catuniverse.gameSupport.BitmapLoader.*;
...
public class Level5 extends TimeLevel {
    ...
    public void run(GamePaint gamePaint) {
        ...
        if (smartObstacles.get(1).isPlayerOn() && !isRequirementsCollected()) {
            PlayerManager.timePlayer.setRocketMode(true);
            super.drawThreeRoadLines(gamePaint);
            if (!oneTime) {
                PlayerManager.timePlayer.setX(30);
                PlayerManager.timePlayer.setY(620);
                oneTime = true;
            }
